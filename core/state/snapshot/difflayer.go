...
func (dl *diffLayer) AccountRLP(hash common.Hash) ([]byte, error) {
	...
	dl.lock.RLock()
	if dl.Stale() {
		log.Warn("snapshot stale on AccountRLP", "root", dl.root, "hash", hash)
		dl.lock.RUnlock()
...
func (dl *diffLayer) accountRLP(hash common.Hash, depth int) ([]byte, error) {
	...
	if dl.Stale() {
		log.Warn("snapshot stale on accountRLP", "root", dl.root, "hash", hash, "depth", depth)
		return nil, ErrSnapshotStale
	}
...
func (dl *diffLayer) Storage(accountHash, storageHash common.Hash) ([]byte, error) {
	...
	// Check staleness before reaching further.
	if dl.Stale() {
		log.Warn("snapshot stale on Storage (public)", "root", dl.root, "account", accountHash, "slot", storageHash)
		dl.lock.RUnlock()
		return nil, ErrSnapshotStale
	}
	var origin *diskLayer
...
func (dl *diffLayer) storage(accountHash, storageHash common.Hash, depth int) ([]byte, error) {
	...
	// the original should be marked as unusable).
	if dl.Stale() {
		log.Warn("snapshot stale on storage (internal)", "root", dl.root, "account", accountHash, "slot", storageHash, "depth", depth)
		return nil, ErrSnapshotStale
	}
